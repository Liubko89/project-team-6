import"./assets/quote-4f246e0e.js";import{a as l,i as b}from"./assets/vendor-db25513e.js";l.defaults.baseURL="https://energyflow.b.goit.study/api";const P=document.querySelector(".exercises-nav-list"),M=document.querySelector(".exercises-container"),U=document.querySelector(".exercise-group"),v=document.querySelector(".pagination"),D="https://energyflow.b.goit.study/api";let L="Muscles";P.addEventListener("click",F);v.addEventListener("click",N);S(L).then(q);async function F(e){const{filter:s}=e.target.dataset;if(!s)return;L=s,await S(s).then(q);const t=P.querySelector(".active"),i=e.target;t.disabled=!1,i.disabled=!0,t.classList.remove("active"),i.classList.add("active")}async function S(e,s=1){return fetch(`${D}/filters?filter=${e}&page=${s}&limit=12`).then(t=>t.json())}function q({results:e,page:s,totalPages:t}){G(e),_(t,s)}function G(e){M.innerHTML=e.map(W).join("")}function W({name:e,filter:s,imgUrl:t}){return`
