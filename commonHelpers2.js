import"./assets/quote-473bab84.js";import{a as l,i as b}from"./assets/vendor-db25513e.js";l.defaults.baseURL="https://energyflow.b.goit.study/api";const P=document.querySelector(".exercises-nav-list"),M=document.querySelector(".exercises-container"),A=document.querySelector(".exercise-group"),x=document.querySelector(".pagination"),D="https://energyflow.b.goit.study/api";let L="Muscles";P.addEventListener("click",F);x.addEventListener("click",N);S(L).then(k);async function F(e){const{filter:s}=e.target.dataset;if(!s)return;L=s,await S(s).then(k);const t=P.querySelector(".active"),i=e.target;t.classList.remove("active"),i.classList.add("active")}async function S(e,s=1){return fetch(`${D}/filters?filter=${e}&page=${s}&limit=12`).then(t=>t.json())}function k({results:e,page:s,totalPages:t}){G(e),_(t,s)}function G(e){M.innerHTML=e.map(W).join("")}function W({name:e,filter:s,imgUrl:t}){return`
  ...
  `}function _(e,s){if(e<=1){x.innerHTML="";return}x.innerHTML=Array(e).fill(1).map((i,r)=>i+r).map(K).join(""),x.children[s-1].classList.add("active")}function K(e){return`
    <li class="page" data-page="${e}">
      <button>${e}</button>
    </li>
  `}function N(e){var t;const s=(t=e.target.closest(".page"))==null?void 0:t.dataset.page;s&&S(L,s).then(k)}M.addEventListener("click",V);function V(e){const t=e.target.closest(".exercise-card");if(!t)return;const i=t.querySelector(".description-category").textContent;A.textContent=i,console.log(i)}const B=document.querySelector(".exercises-nav-list"),E=document.querySelector(".fetch-container"),z=document.querySelector(".exercises-container"),Y=document.querySelector(".exercises-gallery"),$=document.querySelector(".exercises-toolbar"),c=document.querySelector(".pagination-exercises"),w=innerWidth<1440?8:9;let q,h,u,p;c.addEventListener("click",te);const J="https://energyflow.b.goit.study/api/",Q={Muscles:"muscles","Body parts":"bodypart",Equipment:"equipment"},o="is-hidden";z.addEventListener("click",X);E.classList.remove(o);$.classList.add(o);async function X(e){const s=e.target.closest(".exercise-card");if(!s)return;const{filter:t,group:i,page:r}=s.dataset;u=t,p=i,await Z(Q[t],i).then(C),E.classList.add(o),$.classList.remove(o),B.addEventListener("click",n);function n(a){console.log(a.target),a.currentTarget===B&&(E.classList.remove(o),$.classList.add(o))}}async function Z(e,s){return fetch(`${J}exercises?${e}=${s}&limit=1e6`).then(t=>t.json()).then(t=>{q=t.results})}function I({bodyPart:e,target:s,name:t,burnedCalories:i,rating:r,time:n,_id:a}){return`<li class = "list-exercises" id="${a}" data-filter="${t}"><div class="options">
  <div class="box-up">
  <div class="box-left">
  ...
  <button type = "button" class="btn-start-arrow">START<svg class="icon-arrow" width="14" height="14"><use href="./svg/icons.svg#icon-arrow"></use></svg></button>
            ...
            </li>`}function ee(e){h=Math.ceil(q.length/w),c.innerHTML=Array(h).fill(1).map((t,i)=>t+i).map(t=>`<li class="page-exercises" data-page="${t}"><button id="prevPage">${t}</button></li>`).join(""),c.children[e-1].classList.add("active"),se(e)}function se(e){var i,r;for(const n of[...c.children]){const a=+n.dataset.page;a!==1&&a!==h&&(a<e-3||a>e+3)&&n.remove()}const s=(i=c.children[1])==null?void 0:i.dataset.page,t=(r=c.lastElementChild.previousElementSibling)==null?void 0:r.dataset.page;s>2&&c.firstElementChild.after("..."),t<h-1&&c.lastElementChild.before("...")}function C(e=1){const s=(e-1)*w,t=q.slice(s,s+w);Y.innerHTML=t.map(I).join(""),ee(e)}function te(e){var t;const s=+((t=e.target.closest(".page-exercises"))==null?void 0:t.dataset.page);s&&C(s)}const T="/project-team-6/assets/icons-66c1ea1d.svg",y=document.querySelector(".modal-backdrop"),ie=document.querySelector(".exercises-gallery");document.querySelector(".exercise-close-btn");const R="is-open";ie.addEventListener("click",ae);async function ae(e){if(e.target.nodeName!=="BUTTON")return;const s=e.target.closest("li").id;console.dir(s);try{const t=await l.get(`/exercises/${s}`);console.log(t.data),ne(t.data),y.classList.add(R)}catch{}}y.addEventListener("click",re);window.addEventListener("keydown",O);function re(e){e.currentTarget===e.target&&j()}function j(){window.removeEventListener("keydown",O),y.classList.remove(R)}function O(e){e.code==="Escape"&&j()}function ne(e={}){const{_id:s,bodyPart:t,equipment:i,time:r,target:n,burnedCalories:a,gifUrl:g,name:m,popularity:v,rating:f,description:H}=e,U=`<div class="exercises-modal-window">
