import"./assets/modulepreload-polyfill-3cfb730f.js";import{a as c,i as r}from"./assets/vendor-8cce9181.js";c.defaults.baseURL="https://energyflow.b.goit.study/api";const B=document.querySelector(".js-close-menu"),C=document.querySelector(".js-open-menu"),g=document.querySelector(".js-menu-container");B.addEventListener("click",D);C.addEventListener("click",R);function D(){g.classList.add("is-hidden")}function R(){g.classList.remove("is-hidden")}const I="https://energyflow.b.goit.study/api",m=document.querySelector(".quote__backend");async function H(){m.innerHTML='<span class="loader"></span>';try{return(await c.get(`${I}/quote`)).data}catch(e){throw r.error({title:"Wrong",message:"Try again",position:"topRight"}),e}}function _(e,t){return`<p class="quote__backend-text">${t}</p>
    <p class="quote__backend-author">${e}</p>`}async function j(){try{const{author:e,quote:t}=await H(),s={author:e,quote:t,savedDate:new Date().toLocaleDateString()};localStorage.setItem("quote",JSON.stringify(s)),localStorage.setItem("savedDate",s.savedDate),v(e,t)}catch(e){console.error(e)}}function v(e,t){m.innerHTML=_(e,t)}async function f(){const e=JSON.parse(localStorage.getItem("quote")),t=localStorage.getItem("savedDate"),s=new Date().toLocaleDateString();e&&t===s?v(e.author,e.quote):await j()}f();function x(){const e=new Date,t=new Date(e.getFullYear(),e.getMonth(),e.getDate()+1,0,0,0,0)-e;setTimeout(function(){f(),x()},t)}x();const h=document.querySelector(".exercises-nav-list"),U=document.querySelector(".exercises-container"),n=document.querySelector(".pagination"),A="https://energyflow.b.goit.study/api";let l="Muscles";h.addEventListener("click",Q);n.addEventListener("click",W);d(l).then(u);async function Q(e){const{filter:t}=e.target.dataset;if(!t)return;l=t,await d(t).then(u);const s=h.querySelector(".active"),a=e.target;s.disabled=!1,a.disabled=!0,s.classList.remove("active"),a.classList.add("active")}async function d(e,t=1){return fetch(`${A}/filters?filter=${e}&page=${t}&limit=12`).then(s=>s.json())}function u({results:e,page:t,totalPages:s}){P(e),F(s,t)}function P(e){U.innerHTML=e.map(O).join("")}function O({name:e,filter:t,imgUrl:s}){return`
  ...
  <button type = "button" class="btn-start-arrow">START<svg class="icon-arrow" width="14" height="14"><use href="../svg/icons.svg#icon-arrow"></use></svg></button>
            ...
            <p class="options-item"><span class="hid-txt">Burned calories:</span>${a}/${o}</p>
            <div class="info-ex">
            <p class="options-item"><span class="hid-txt">Body part:</span>${e}</p>
            <p class="options-item"><span class="hid-txt">Target:</span>${t}</p></div>
            </div>
            </li>`}function $(e){J.innerHTML=e.map(K).join("")}const w=document.querySelector(".modal-backdrop"),V=document.querySelector(".exercises-gallery");document.querySelector(".exercise-close-btn");const Y="is-hidden";V.addEventListener("click",X);async function X(e){if(e.target.nodeName!=="BUTTON")return;const t=e.target.closest("[data-filter]").dataset.filter;(await c.get("https://energyflow.b.goit.study/api/exercises").then(a=>a.data.results).catch(a=>console.log(a))).map(a=>{a.name===t&&(w.classList.remove(Y),Z(a))})}function Z(e={}){const{_id:t,bodyPart:s,equipment:a,time:i,target:o,burnedCalories:S,gifUrl:E,name:L,popularity:q,rating:M,description:T}=e,k=`<div class="exercises-modal-window">
