import"./assets/quote-473bab84.js";import{a as l,i as b}from"./assets/vendor-db25513e.js";l.defaults.baseURL="https://energyflow.b.goit.study/api";const P=document.querySelector(".exercises-nav-list"),M=document.querySelector(".exercises-container"),U=document.querySelector(".exercise-group"),f=document.querySelector(".pagination"),D="https://energyflow.b.goit.study/api";let L="Muscles";P.addEventListener("click",F);f.addEventListener("click",N);S(L).then(q);async function F(e){const{filter:t}=e.target.dataset;if(!t)return;L=t,await S(t).then(q);const s=P.querySelector(".active"),i=e.target;s.classList.remove("active"),i.classList.add("active")}async function S(e,t=1){return fetch(`${D}/filters?filter=${e}&page=${t}&limit=12`).then(s=>s.json())}function q({results:e,page:t,totalPages:s}){G(e),_(s,t)}function G(e){M.innerHTML=e.map(W).join("")}function W({name:e,filter:t,imgUrl:s}){return`
  ...
  <button type = "button" class="btn-start-arrow">START<svg class="icon-arrow" width="14" height="14"><use href="./svg/icons.svg#icon-arrow"></use></svg></button>
            ...
            </li>`}function ee(e){h=Math.ceil(k.length/w),n.innerHTML=Array(h).fill(1).map((s,i)=>s+i).map(s=>`<li class="page-exercises" data-page="${s}"><button id="prevPage">${s}</button></li>`).join(""),n.children[e-1].classList.add("active"),te(e)}function te(e){var i,r;for(const c of[...n.children]){const a=+c.dataset.page;a!==1&&a!==h&&(a<e-3||a>e+3)&&c.remove()}const t=(i=n.children[1])==null?void 0:i.dataset.page,s=(r=n.lastElementChild.previousElementSibling)==null?void 0:r.dataset.page;t>2&&n.firstElementChild.after("..."),s<h-1&&n.lastElementChild.before("...")}function C(e=1){const t=(e-1)*w,s=k.slice(t,t+w);Y.innerHTML=s.map(I).join(""),ee(e)}function se(e){var s;const t=+((s=e.target.closest(".page-exercises"))==null?void 0:s.dataset.page);t&&C(t)}const T="/project-team-6/assets/icons-66c1ea1d.svg",y=document.querySelector(".modal-backdrop"),ie=document.querySelector(".exercises-gallery");document.querySelector(".exercise-close-btn");const R="is-open";ie.addEventListener("click",ae);async function ae(e){if(e.target.nodeName!=="BUTTON")return;const t=e.target.closest("li").id;console.dir(t);try{const s=await l.get(`/exercises/${t}`);console.log(s.data),ce(s.data),y.classList.add(R)}catch{}}y.addEventListener("click",re);window.addEventListener("keydown",O);function re(e){e.currentTarget===e.target&&j()}function j(){window.removeEventListener("keydown",O),y.classList.remove(R)}function O(e){e.code==="Escape"&&j()}function ce(e={}){const{_id:t,bodyPart:s,equipment:i,time:r,target:c,burnedCalories:a,gifUrl:m,name:g,popularity:x,rating:v,description:A}=e,H=`<div class="exercises-modal-window">
        ...
        </div>`;y.innerHTML=H}const ne=document.querySelector(".form-search");ne.addEventListener("submit",ue);function oe(e,t,s=1,i=9){return l.get("/exercises",{params:{bodypart:e,keyword:t,page:s,limit:i}})}function le(e,t,s=1,i=9){return l.get("/exercises",{params:{muscles:e,keyword:t,page:s,limit:i}})}function de(e,t,s=1,i=9){return l.get("/exercises",{params:{equipment:e,keyword:t,page:s,limit:i}})}let d;async function ue(e){e.preventDefault(),d=e.currentTarget.elements["input-search"].value.trim(),console.log(d),console.log(u),console.log(p);let t;try{u==="Body parts"?t=await oe(p,d):u==="Muscles"?t=await le(p,d):u==="Equipment"&&(t=await de(p,d));const{totalPages:s,perPage:i,results:[{bodyPart:r,target:c,name:a,burnedCalories:m,rating:g,time:x,_id:v}]}=t.data;console.log(t),console.log(r,c,a,m,g,x,v),t.data.results.length===0?console.log("TODO: Unfortunately, no results were found. You may want to consider other search options to find the exercise you are looking for. Our range is wide and you have the opportunity to find more options that suit your needs."):(console.log("TODO: малюэмо розмітку"),I({bodyPart:r,target:c,name:a,burnedCalories:m,rating:g,time:x,_id:v}),C())}catch(s){console.log(s)}finally{}}document.getElementById("footerForm").addEventListener("submit",function(e){e.preventDefault();const t=document.getElementById("emailInput").value;if(!pe(t)){b.error({title:"Error",message:"Please enter a valid email address",position:"topRight"});return}l.post("https://energyflow.b.goit.study/api/subscription",{email:t},{headers:{"Content-Type":"application/json"}}).then(function(s){console.log(s.data),b.success({title:"Success",message:"Subscription created successfully!",position:"topRight"}),document.getElementById("emailInput").value=""}).catch(function(s){console.error("Error:",s),b.error({title:"Error",message:"Failed to create subscription",position:"topRight"})})});function pe(e){return/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)}
